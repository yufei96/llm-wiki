...
MOSA的安全性讨论长期停留在"开放性vs安全性"的**政策张力**层面（[[cybersecurity-vs-mosa-openness]]）。NIST SP 800-160 Vol. 2 Rev. 1 提供了**架构层面的解决方案**——11项结构性设计原则中，有7项与MOSA的模块化架构直接对齐。这不是巧合：模块化本身就是一种安全架构模式。
...
2. **从"张力"到"协同"**：之前讨论的"开放性vs安全性"张力（[[cybersecurity-vs-mosa-openness]]）在架构层面可以转化为**协同**——开放的接口 + 模块化架构 = 安全性
...
- [[nist-sp-800-160-v2-cyber-resilience]] — NIST赛博弹性框架（来源）
- [[cybersecurity-vs-mosa-openness]] — 赛博安全与MOSA开放性的政策张力
- [[interface-engineering-evolution]] — 接口工程三层演进
- [[nasa-se-handbook-rev2]] — NASA接口管理工程实践
- [[sysml-v2-specification]] — SysML v2（形式化建模NIST原则的工具）
