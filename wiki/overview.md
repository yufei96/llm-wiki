...
- **1994**: [[OSJTF-Charter-1994|OSJTF宪章]]签署，MOSA理念系统化（Paul G. Kaminski, USD(A&T)）
- **2000**: [[DoD-IG-OSJTF-Audit-2000|DoD IG审计报告]]揭示MOSA实施早期挑战
...
| 层 | 文件夹 | 数量 | 功能 |
|----|--------|------|------|
| 来源 | sources/ | 88 | 一手文档摘要 |
| 概念 | concepts/ | 39 | 单一概念定义 |
| 实体 | entities/ | 22 | 组织/人物/政策 |
| 主题 | topics/ | 15 | 跨来源综合 |
| 对比 | comparisons/ | 20 | 演化/张力/趋同分析 |

### 10. 中国智能网联汽车与无人机开放生态

将MOSA原则与中国产业实践对照，提供跨国别、跨领域的开放系统架构参考：

- **车路云一体化**（[[china-vehicle-road-cloud-integration]]）：20个试点城市、云控平台两级架构（边缘云+区域云）、跨行业标准体系8部分、十大功能场景
- **华为车辆开放生态**（[[huawei-vehicle-open-ecosystem]]）：OpenHarmony开源基座、iDVP数字底座、Car Kit开放API、三种合作模式（零部件/HI/鸿蒙智行）
- **大疆开放无人机架构**（[[dji-open-drone-architecture]]）：Mobile SDK/Payload SDK/Onboard SDK/Edge SDK/Cloud API五层开放体系，抽象组件化设计实现即插即用

**对比洞察**：中国产业界的开放架构实践在"平台自有、生态开放"的层面与MOSA的"政府定架构、供应商竞争"存在本质差异，但技术原则高度一致——模块化、接口标准化、即插即用、跨产品兼容。

### 11. 解放军开放系统架构：有限信息的拼图

鲜有顶层政策文件公开，通过碎片化文献可拼凑出五个层面的实践（[[pla-open-system-architecture]]）：
...
与美军核心差异：①语言差异（"三化"vs MOSA术语）②驱动力差异（学术预研vs法律强制）③透明度差异（预研项目vs完全公开）。详见十维对比：[[china-vs-us-open-architecture]]。
