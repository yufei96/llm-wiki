---
title: "SAM.gov UAI/CFW ICD RFI"
created: 2026-05-07
updated: 2026-05-07
tags: [UAI, CFW, ICD, 武器接口, SAM.gov]
source:
  - raw/papers/samgov-uai-rfi.pdf
  - raw/articles/samgov-uai-rfi.md
---

# SAM.gov UAI/CFW ICD RFI

## 摘要

美国政府在SAM.gov（联邦采购数据系统）发布的通用武器接口（UAI）/通用火控武器（CFW）接口控制文档（ICD）信息征询书。这是美国军方通过正式采购渠道公开征求行业对开放式武器接口标准的反馈，体现MOSA在武器接口标准化方面的推进。325KB原始PDF，378行转换文本。

## 关键信息

- 联邦采购系统公开发布
- 征求行业对开放武器接口标准的反馈
- UAI/CFW ICD标准化需求

## 原文来源

- https://sam.gov/opp/71a7c60afc4546d39f6c7f73944c215b/view
- 原始文件: `raw/papers/samgov-uai-rfi.pdf`, `raw/articles/samgov-uai-rfi.md`

## 相关内容

- [[open-architecture-hierarchy]] — 开放架构层次体系
- [[wosa-weapons-open-systems-architecture]] — WOSA武器开放系统架构
