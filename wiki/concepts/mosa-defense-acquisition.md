...
MOSA的"出生证明"是1994年11月29日Paul G. Kaminski（时任USD(A&T)）签署的OSJTF宪章。宪章原件为4页扫描件，通过OCR提取入库。

关键历史节点：
- **1994年6月29日**：国防部长发布"规范与标准——新的做事方式"备忘录，确立标准化优先顺序：性能 > 商业 > 军用
- **1994年9月16日**：开放系统工作组提交实施建议
- **1994年11月29日**：Kaminski签署OSJTF备忘录和宪章，成立OSJTF
- **宪章核心**：OSJTF是陆军、海军、空军和USD(A&T)的合作努力，重点是武器系统和平台（不包括C3I系统），不会强制通用硬件，而是在允许物理硬件定制的层次上标准化
- **宪章有效期**：4年（1994-1998）

Kaminski备忘录原文（OCR）：
> "开放系统规范和标准是基于共识的、公开的或非专有的规范和标准，用于硬件、软件、工具和架构的系统及接口。"

原始文档：[[OSJTF-Charter-1994]]
...
- [[three-se-handbooks-detailed-comparison]] — 三本SE手册对比
- [[defense-acquisition-overrun-trends]] — 采办项目绩效危机
- [[face-technical-standard]] — FACE航电标准（MOSA成功案例）
- [[open-mission-systems]] — OMS航空任务系统标准
- [[better-buying-power-3-0]] — MOSA首次写入DoD政策（2015）
- [[modular-architecture-patterns]] — 模块化架构模式
- [[middle-tier-acquisition]] — MTA快速采办路径
- [[ucs-control-station]] — 无人系统指挥控制标准
- [[wosa-architecture-tools]] — WOSA架构评估工具链
- [[ppi-did-templates]] — PPI DID模板（合同执行层）
- [[mosa-implementation-stack]] — MOSA实施全栈（五层架构）
- [[requirements-quality-mosa]] — 需求质量：MOSA技术基础
