...
**现实证据：** GAO 多次报告指出，MOSA 合规性缺乏标准化度量，PM 可以声称 MOSA 合规而实际锁定。DoDI 5000.85 要求"最大程度可行范围内"实施 MOSA，但未定义"最大程度"是什么。→ [[mosa-five-pillars]]、[[requirements-quality-mosa]]
...
**现实证据：** SysML v1 的 XML 互操作——标准有了，但 14 家供应商各自解释，实际互操作为零。OMS 标准 PDF 损坏后无人维护，说明标准生命力依赖单一组织。→ [[sysml-v2-transition-bottleneck]]、[[ousd-sysml-v2-transition-guidance]]
...
**现实证据：** F-35 的模块化设计在实际升级中发现，航电/任务系统/传感器的耦合远超预期，模块替换成本接近重新设计。→ [[modular-architecture-patterns]]、[[interface-engineering-evolution]]
...
**现实证据：** MOSA 成本效益数据极度匮乏。GAO 报告承认"缺乏归因数据"——没有人能量化 MOSA 到底省了多少钱。→ [[mosa-implementation-stack]]
...
**现实证据：** 中间采办层级（MTA）路径选择跳过 MOSA 要求以加速交付——说明 MOSA 在"加速"维度上被视为阻碍而非助力。→ [[middle-tier-acquisition]]、[[speed-vs-modularity]]
...
这不是实现问题，是**结构性不可能**：你不能同时拥有"每个零件都能随便换"和"整个系统被最优地调谐"。MOSA 五支柱中的"模块化"和"任务效能"本质上在争夺同一个自由度。→ [[mosa-five-pillars]]
...
你不能同时快速交付和严格验证 MOSA 合规——而 AAF 的 MTA 路径选择了速度。→ [[adaptive-acquisition-framework]]、[[speed-vs-modularity]]
...
- [[mosa-five-pillars]] — MOSA 五大支柱（本分析的反转对象）
- [[speed-vs-modularity]] — 速度与模块化的张力（守恒律的具体表现）
- [[requirements-quality-mosa]] — 需求质量（竞争假象的根源之一）
- [[middle-tier-acquisition]] — MTA 路径（合规壁垒的现实选择）
- [[adaptive-acquisition-framework]] — AAF 六路径（速度/合规的制度化分流）
- [[mosa-implementation-stack]] — MOSA 实施全栈（隐性开销的分布位置）
